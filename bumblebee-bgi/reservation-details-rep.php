...
if(!empty($reservation[42])){
    $reps = json_decode($reservation[42]);
}
...
//Sanitize data
    ...
    if(!empty($rep_name) and is_array($rep_name)){ $rep_name = json_encode($rep_name); }
    if (!empty($rep_name)){
    $assigned = 1;
    } else {
    $assigned = 0;
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="form-group col-xs-2"><!-- Rep assignment -->
                                    <?php
                                            ...
                                            echo '<select class="form-control select" multiple="multiple" id="rep-name" name="rep_name[]">';
                                            if(empty($reps))
                                            echo '<option value="0" selected >Select a Rep</option>';
                                            while ($row = mysql_fetch_array($repselect)) {
                                                if (in_array($row['id_rep'], $reps)) {
