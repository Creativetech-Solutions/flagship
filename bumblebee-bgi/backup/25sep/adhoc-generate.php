...
if(isset($TotalRows) and $TotalRows > 0){
    ...
    while($row = mysqli_fetch_assoc($queryResource)) {
        ...
        if(isset($row['Reps']) && is_array($row['Reps']) && !empty($row['Reps'])){
            $ids = $row['Reps'];
            $ids = implode("','",$ids);
            $query = "SELECT `name` FROM bgi_reps WHERE id_rep IN ('".$ids."')"; 
    ...
    $guestCols=['Guest_Title_Name','Guest_First_Name','Guest_Last_Name','Guest_Email','Guest_Cell','Guest_PNR','Guest_Adult','Guest_Child_Age','Guest_Infant_Age','Price','Guest_id'];
