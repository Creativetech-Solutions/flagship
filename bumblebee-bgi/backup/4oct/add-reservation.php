    ...
    if(!empty($guestsArray)){
        foreach($guestsArray as $guest){
            $sqlGuestQuery = "INSERT INTO bgi_guest ".
                "(ref_no_sys, title_name, first_name, last_name, adult, child_age, infant_age, pnr, email, cell) ".
                "VALUES ('$fsref', '".$guest['guestTitle']."', '".$guest['guestFirstName']."', '".$guest['guestLastName']."', '".(!empty($guest['guestAdult'])?1:0)."', '".$guest['guestChildAge']."', '".$guest['guestInfantAge']."', '".$guest['guestPNR']."', '".$guest['guestEmail']."', '".$guest['guestCell']."')";
