...
if(isset($_POST['name']) || isset($_POST['arr_date']) || isset($_POST['dept_date'])){
    ...
    if(isset($_POST['name']) && !empty($_POST['name'])){
        $reservationQuery .= " AND (R.first_name LIKE '%".$_POST['name']."%' OR R.last_name LIKE '%".$_POST['name']."%' OR R.tour_ref_no LIKE '%".$_POST['name']."%' OR R.flight_class LIKE '%".$_POST['name']."%' OR R.ref_no_sys LIKE '%".$_POST['name']."%' OR R.dpt_notes LIKE '%".$_POST['name']."%' OR R.arr_transport_notes LIKE '%".$_POST['name']."%')";

    } else $_POST['name'] = '';

    if (isset($_POST['arr_date']) && !empty($_POST['arr_date'])){
        $reservationQuery .= " AND R.arr_date = '".date('Y-m-d', strtotime($_POST['arr_date']))."'";
    } else $_POST['arr_date'] = '';
...
//Grab all reservation info
$reservations = mysql_query('SELECT * '.$reservationQuery." GROUP BY R.id ORDER BY R.id DESC LIMIT 10 OFFSET ".$offset);
/*echo '<pre>';
...
$totalRows = mysql_fetch_row(mysql_query("SELECT COUNT(R.id) ".$reservationQuery));
                                    ...
                                    <table id="res-arrivals" class="table table-hover display">
                                        ...
                                        <?php 
                                            while($row = mysql_fetch_array($reservations)) {
                                                ...
                                                $actualhotel = mysql_fetch_row(mysql_query("SELECT * FROM bgi_location WHERE id_location='" . $row[71] . "'"));
