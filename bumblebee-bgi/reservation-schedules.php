                                    ...
                                    <?php if ($user->levelCheck("2,9")) : ?>
                                        ...
                                        <?php
                                            while($row = mysql_fetch_array($reservations)) {
                                                ...
                                                $rep ='';
                                                if(!empty($row[42])){
                                                    $reps = json_decode($row[42], true); 
                                                    $repIds = implode("','",$reps);
                                                    $repsQuery = mysql_query("SELECT * FROM bgi_reps WHERE id_rep IN ('".$repIds."')");
                                                    $allReps = [];
                                                    while($rep = mysql_fetch_array($repsQuery)){
                                                        if(isset($rep[1])){
                                                            $allReps[] = $rep[1];
                                                        } 
                                                    } 
                                                ...
                                                echo '<tr>
                                                        <td><a href="reservation-details-rep.php?id=' . $id . '"><i class="fa fa-search" data-toggle="tooltip" data-placement="top" title="Click to change rep: ' . $rep . '"></i></a>' . $displayft . '</div></td>
                                                        <td>' . $rep . '</td>
                                                        <td>' . $ref_no . '</td>
                                                        <td>' . $title_name . '</td>                                                        
                                                        <td>' . $first_name . '</td>
