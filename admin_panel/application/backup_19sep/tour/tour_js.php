  ...
  $(function () {
    ...
    function delTourOp(opId){
     ...
     $('#edit-tour').on('show.bs.modal', function (e) {
      <?php //check the modal invoker element, as same modal will be use for different functionalities(add, edit) ?>
        ...
        if((invoker.attr('class')).indexOf(' edit') > -1) {
          ...
          var rep_cell=invoker.parents('.tour_operator').find('.rep_cell').html();
          var op_name = op_name.replace(/&amp;/g, '&');
          $(this).find('.op_name').val(op_name);
          $(this).find('.rep_cell').val(rep_cell);
     ...
     $(document).on('click','.save-tour', function(){
        ...
        var rep_cell = parentMdl.find('.rep_cell').val();
        // console.log(op_name+' dd'+opId);
        if(op_name!=''){
          $.ajax({
            url:"<?=base_url('touroperator/saveOperator')?>",
            type:"POST",
            data:{opId:opId,op_name:op_name,rep_cell:rep_cell},
            beforeSend:function(){},
            success:function(data){
              ...
              if(data[2]=='success'){
                ...
                if(data[3]=='update'){
                  ...
                  $('.tour_operator[data-id="'+opId+'"]').find('.rep_cell').html(rep_cell);
                }
                // if new record added
                else if(data[3]=='add'){
                  ...
                  var rowNode = table.row.add( [ '', record['id'], record['tour_operator'], record['cell'] ,'<a class="btn btn-sm btn-primary edit" data-toggle="modal" data-target="#edit-tour"><i class="fa fa-pencil"></i> Edit</a>', '<a data-id="del-tourop" class="btn btn-sm btn-danger del-tourop" data-toggle="modal" data-target="#confirm_modal"><i class="fa fa-trash"></i> Delete</a>' ] ).draw().node();
