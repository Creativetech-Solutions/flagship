...
/*$ftnotify = mysql_fetch_row(mysql_query("SELECT `ftnotify` FROM skb_reservations WHERE id = '$reservation_id'"));
if(isset($ftnotify[0])) $ftnotify = $ftnotify[0]; 
else $ftnotify='';*/
$getArrivalQuery = "SELECT * FROM skb_arrivals WHERE id='" . QuoteSmart($_GET['arrival_id']) . "'";
//$reservation = mysql_fetch_row(mysql_query($getArrivalQuery));
$reservation = mysql_fetch_row(mysql_query($getArrivalQuery));
$get_arr_flight_no = mysql_fetch_row(mysql_query("SELECT * FROM skb_flights WHERE id_flight='" . $reservation[4] . "'"));
$get_arr_time = mysql_fetch_row(mysql_query("SELECT * FROM skb_flighttime WHERE id_fltime='" . $reservation[3] . "'"));
$get_arr_pickup = mysql_fetch_row(mysql_query("SELECT * FROM skb_location WHERE id_location='" . $reservation[9] . "'"));  
$get_arr_location = mysql_fetch_row(mysql_query("SELECT * FROM skb_location WHERE id_location='" . $reservation[10] . "'"));
$get_arr_roomtype = mysql_fetch_row(mysql_query("SELECT * FROM skb_roomtypes WHERE id_room='" . $reservation[11] . "'"));  
$get_arr_driver = mysql_fetch_row(mysql_query("SELECT * FROM skb_transport WHERE id_transport='" . $reservation[7] . "'"));
$get_arr_vehicle = mysql_fetch_row(mysql_query("SELECT * FROM skb_vehicles WHERE id_vehicle='" . $reservation[8] ."'"));
$get_flightclass = mysql_fetch_row(mysql_query("SELECT * FROM skb_flightclass WHERE id='" . $reservation[5] . "'"));
$selectedFastTrack = $reservation[26];

//Need to get rep type.
$repTypeQuery = "SELECT * FROM skb_reptype WHERE id IN (" . $reservation[12] . ")";
...
$roomtypeselect = mysql_query("SELECT * FROM skb_roomtypes WHERE id_location='" . $reservation[10] . "' ORDER BY id_room ASC");
...
//Sanitize data
   ...
   // echo '<pre>'; print_r($_POST); echo '</pre>'; exit;
    ...
    $retval1 = mysql_query( $sql_1, $conn );
     ...
     if($ftnotify == 1){
        $sql_12 = "UPDATE skb_reservations SET `ftnotify` = 1 WHERE ref_no_sys = '$reservation[1]'";
        mysql_query( $sql_12, $conn );
    }
    
    //Log user action
    $sql_2 = "INSERT INTO skb_activity ". 
        "(log_user, user_action, log_time) ". 
        "VALUES ('$loggedinas', '$user_action', NOW())";
        $retval2 = mysql_query( $sql_2, $conn ); 
       
   //Update system log
    $sql_3 = "UPDATE skb_reservations ". 
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="form-group col-xs-7">                                        
                                        <select multiple class="form-control select" id="client-reqs" name="client_reqs[]">                                      
                                            ...
                                            <?php
                                            $sql = "SELECT * FROM skb_clientreqs";
                                            if($section == 'gh'){
