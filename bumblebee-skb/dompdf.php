...
include('header.php');
	...
	where `table_schema` = 'cocoa_skb' and `table_name` in ('skb_departures', 'skb_arrivals',  'skb_reservations')");
	...
	if(isset($_REQUEST['sect']) && $_REQUEST['sect']=='fsft'){
	    $reservations = mysql_query("SELECT * FROM skb_reservations as res
	    left join skb_arrivals as arr on res.arr_flight_no = arr.id
	    left join skb_departures as dep on res.arr_flight_no = dep.id
	...
	while($row = mysql_fetch_array($reservations)) {
                                                ...
                                                $rep_type = mysql_fetch_row(mysql_query("SELECT * FROM skb_reptype WHERE id='" . $row[24] . "'"));
                                                $flight_class = mysql_fetch_row(mysql_query("SELECT * FROM skb_flightclass WHERE id='" . $row[17] . "'"));
                                                $rep_name = mysql_fetch_row(mysql_query("SELECT * FROM skb_reps WHERE id_rep='" . $row[42] . "'"));
                                                $hotel = mysql_fetch_row(mysql_query("SELECT * FROM skb_location WHERE id_location='" . $row[22] . "'"));
                                                $arr_driver = mysql_fetch_row(mysql_query("SELECT * FROM skb_transport WHERE id_transport='" . $row[19] . "'"));
                                                $dep_driver = mysql_fetch_row(mysql_query("SELECT * FROM skb_transport WHERE id_transport='" . $row[30] . "'"));
