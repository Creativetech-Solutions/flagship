...
$selectedDateFlights_sql = "SELECT ff.id_flight AS flightNumberID, ff.flight_number  from $table INNER JOIN skb_flights ff ON `ff`.`id_flight` = flight.$column where `$whereColumn` = '$filterDate' AND `$column` != 0 GROUP BY ff.id_flight";
