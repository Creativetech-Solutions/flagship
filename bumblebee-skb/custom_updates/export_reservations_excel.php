...
$reservations = mysqli_query($conn , "SELECT 
  ...
  CONCAT(skb_guest.title_name, ' ' ,skb_guest.first_name, ' ', skb_guest.last_name) AS Guest,
  skb_guest.adult AS Guest_Adult,
  skb_guest.child_age AS Guest_Child_Age,
  skb_guest.infant_age AS Guest_Infant_Age,
  skb_guest.pnr AS Guest_PNR,
  ...
  skb_flights.flight_number AS Arr_Flight,
  ...
  FDV.name AS DptVehicle
   ...
   LEFT JOIN skb_guest ON skb_guest.ref_no_sys = R.ref_no_sys
   LEFT JOIN skb_arrivals FA ON FA.ref_no_sys = R.ref_no_sys
   LEFT JOIN skb_flightclass FC ON FA.flight_class = FC.id
   LEFT JOIN skb_transport FAD ON FAD.id_transport = FA.arr_driver
   LEFT JOIN skb_vehicles FAV ON FAV.id_vehicle = FA.arr_vehicle
   LEFT JOIN skb_departures FD ON FD.ref_no_sys = R.ref_no_sys
   LEFT JOIN skb_flightclass FDC ON FDC.id = FD.flight_class
   LEFT JOIN skb_transport FDD ON FDD.id_transport = FD.dpt_transport
   LEFT JOIN skb_vehicles FDV ON FDV.id_vehicle = FD.dpt_vehicle
   LEFT JOIN skb_flights DFN ON DFN.id_flight = FD.dpt_flight_no
    WHERE R.status = 1
   ");
