...
if(isset($_POST['action']) && $_POST['action']=='delete' && isset($_POST['reportId'])){
	$response = mysqli_query($conn, "DELETE FROM skb_reports WHERE id=".$_POST['reportId']);
	if($response){
...
if(isset($_POST['reportId']) && !empty($_POST['reportId'])){
	...
	$response = mysqli_query($conn, "UPDATE skb_reports SET `name`='$reportName', `setting`='$reportSettings',`user_id`='$userid' WHERE `id`='$rep_id'");
	...
	$response = mysqli_query($conn, "INSERT INTO skb_reports (`name`, `setting`, `user_id`, `fsft`, `created_date`) VALUE ('$reportName', '$reportSettings', '$userid', '$fsft', '$date')");
